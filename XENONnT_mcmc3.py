...
def B_integrand(radius, w, ms2_arr, mg2_arr, g1_arr, g2_arr):  # [eV Bg-2] Eq.7 in paper 2406.01691
    ...
    for i in range(n):
        ...
        results[i] = 2 * r**2.0 * B**2.0 / (pi * Mpl * Mpl) * factor1 / factor2  # in eV
    return results
...
# efficiency = np.loadtxt("XENON1T/Efficiency.dat")  ## for XENON1T
...
omp = np.geomspace(0.01, 20.0, 200)   ## in keV
def Rate_cham(wkeV_array, ge, gg, logMe, gc, n, cross_sec, epsilon, use_skew=False):   
    result = np.zeros_like(wkeV_array)
    for j in range(len(wkeV_array)):
        wkeV = wkeV_array[j]
        weV = 1e3 * wkeV  ## we use [eV] unit during the chameleon production calculation
...
def log_likelihood(theta, energy_bin, events, err, cross_sec, epsilon):   ## Calculate the chi-squared value
    ...
    event_theory = Rate_cham(energy_bin, ge, gg, logMe, gc, n, cross_sec, epsilon)  ## energy_bin in [keV]
    chi2_value = 0.0
...
initial = np.array([2.0, 7.8, 3.95, -6.0, 1.0])  # initial guess for parameters
...
if __name__ == "__main__":
    ...
    np.savez("chains/XENON1T_chain_50000.npz", chain=sampler.get_chain(), log_prob=sampler.get_log_prob())
    flat_samples = sampler.get_chain(flat=True)  # flatten the chain
    # with open("chains/XENONnT_mcmc_50000.json", "w") as f:
    #     json.dump(flat_samples.tolist(), f)  # save the samples to a JSON file

    print("MCMC sampling completed and saved to XENONnT_mcmc_100000.json")   # Print completion message

    # Plot the results
    figure = corner.corner(flat_samples, labels=["ge", "gg", "logMe", "gc", "n"], 
                       quantiles=[0.16, 0.5, 0.84], 
                       show_titles=True, smooth=True, title_kwargs={"fontsize": 15})
    plt.savefig("chains/XENON1T_mcmc3_50000.pdf")  # Save the corner plot
    plt.show()  