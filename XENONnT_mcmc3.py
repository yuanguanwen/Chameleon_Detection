...
initial = np.array([1.0, 10.0, 3.90, -3.0, 2.0])  # initial guess for parameters
...
if __name__ == "__main__":
    ...
    np.savez("chains/chain_50000.npz", chain=sampler.get_chain(), log_prob=sampler.get_log_prob())
    flat_samples = sampler.get_chain(flat=True)  # flatten the chain

    print("MCMC sampling completed and saved to mcmc_50000.npz")   # Print completion message

    # Plot the results
    figure = corner.corner(flat_samples, labels=["ge", "gg", "logMe", "gc", "n"], 
                       quantiles=[0.16, 0.5, 0.84], 
                       show_titles=True, smooth=True, title_kwargs={"fontsize": 15})
    plt.savefig("chains/mcmc3_50000.pdf")  # Save the corner plot
    plt.show()  