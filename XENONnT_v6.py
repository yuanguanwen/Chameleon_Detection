...
Mpl = 1.221e28                  # planck mass in eV
...
def mCham2(r, ge=2.0, gc=-3.0, n=1.0):  # effective mass square, Eq.3-4 in 1110.2583
    phim = (n * Mpl * 10**((4. + n)*gc - ge) / rho_r(r))**(1. / (1. + n))
    return (1. + n) * 10**ge * rho_r(r) / Mpl / phim  # [eV2]

############################# Bulk Magnetic Field #############################
@njit
def Bsolar(radius_arr, Bup=True):   ## radius in eV
...
w_array = np.linspace(0.1, 28, 50)  # [keV]
events_mod1 = Rate_cham(w_array, ge=2.0, gg=10.0, logMe=3.98, gc=-5.0, n=1, use_skew=False)  # Gaussian resolution
events_mod2 = Rate_cham(w_array, ge=1.0, gg=10.0, logMe=3.95, gc=-5.0, n=1, use_skew=False)  # Gaussian resolution
events_mod3 = Rate_cham(w_array, ge=10.0, gg=10.0, logMe=3.95, gc=-5.0, n=1, use_skew=False)  # Gaussian resolution
# Nexp1 = eX1T*np.trapz(events_mod1, w_array)
# Nexp2 = eX1T*np.trapz(events_mod2, w_array)
...
plt.errorbar(bin30_centers, count30/1.16, yerr=err30/1.16, fmt='o', color='black', capsize=3, label=r'${\rm Data}$')
plt.plot(x2, y2, color='red', linewidth=2.5, linestyle="-",  label=r"${\rm B_0}$")
plt.plot(w_array, events_mod1+background(w_array), label=r"${\rm B_0+Chameleon}$", color='blue', linestyle="--", linewidth=2.5)
# plt.plot(w_array, events_mod2+background(w_array), label=r"${\rm B_0, gg=9}$", color='purple', linewidth=2.5)
# plt.plot(w_array, events_mod3+background(w_array), label=r"${\rm B_0, gg=8}$", color='yellow', linewidth=2.5)
...
plt.tight_layout()
plt.savefig("plots/XENONnT_v6.pdf", dpi=300, bbox_inches='tight')
plt.show()
