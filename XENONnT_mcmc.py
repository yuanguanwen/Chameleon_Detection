...
cm2eV = 1.973269804e-5       # cm-1 to eV
Dist = 1.496e13              # cm, average distance between Earth and Sun
rSolar = 6.957e10/ cm2eV     # solar radius [eV-1]
...
# @njit
def mCham2(r, ge=2.0, gc=-3.0, n=1.0):  # effective chameleon mass square, Eq.3-4 in 1110.2583, gc -> log10(Lambda/eV)
...
def Bsolar(radius_arr, Bup=False):   ## radius in eV
    ...
    # B = [1600.0*T2eV, 27.0*T2eV, 3.5*T2eV]
...
def selectG(Tr, w):  
    Tnorm = Tr/me
    idx_T1 = np.searchsorted(z1[0,1:], Tnorm)
    idx_T2 = np.searchsorted(z2[0,1:], Tnorm)
    idx_w1 = np.searchsorted(z1[1:,0]*Tr, w)
    idx_w2 = np.searchsorted(z2[1:,0]*Tr, w)

    g1 = z1[idx_w1+1, idx_T1+1]
    g2 = z2[idx_w2+1, idx_T2+1]
    return g1, g2
...
def cross_sec(w, ge, logMe): # w in eV, logMe -> log10(M_e/eV)
    ...
    return conf + dis*NXe   ### unit in cm^2/g


# XENONnT efficiency in KeV
efficiency = np.loadtxt("XENONnT/efficiency.txt")  ## for XENONnT
...
def Rate_cham(wobs_array, ge, gg, logMe, gc, n, use_skew=False):   
    w_true_min, w_true_max = 0.01, 40.0 
    w_true_grid = np.geomspace(w_true_min, w_true_max, 400) # keV
    
    flux_vals = np.array([flux_earth(w * 1e3, ge, gg, gc, n) for w in w_true_grid])
    sigma_vals = np.array([cross_sec(w * 1e3, ge, logMe) for w in w_true_grid])
    physics_true = flux_vals * sigma_vals   
    result = np.zeros_like(wobs_array)
    
    for j, w_obs in enumerate(wobs_array):
        if use_skew:
            res_kernel = np.array([res_sgm_skew(w_obs, wt) for wt in w_true_grid])
        else:
            res_kernel = np.array([res_sgm(w_obs, wt) for wt in w_true_grid])
        
        integrand = physics_true * res_kernel
        result[j] = epsilon(w_obs) * conv * np.trapz(integrand, w_true_grid)
        
    return result


############################### emcee for MCMC ###############################
def log_likelihood(theta, energy_bin, events, err):   ## Calculate the chi-squared value
    ge, gg, logMe, gc, n = theta
    event_theory = Rate_cham(energy_bin, ge, gg, logMe, gc, n)  ## energy_bin in [keV]
    # event_theory = fast_rate_cham(theta, w_true_grid, res_matrix)  ## energy_bin in [keV]
    chi2_value = 0.0
...
def log_prior(theta):  ## logprior is flat in default 
    ...
    if ge < 0.0 or ge > 2.0 or \
       gg < 0.0 or gg > 15.0 or \
       logMe < 0.0 or logMe > 6.0 or \
       gc < -8.0 or gc > 0.0 or \
       n < 0.0 or n > 6.0:
        return -np.inf
    return 0.0
...
def log_posterior(theta, energy_bin, events, err):
    lp = log_prior(theta)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood(theta, energy_bin, events, err)

## create log_prob function with fixed arguments
log_prob = partial(log_posterior, energy_bin=energy_bin, events=events, err=err)

ndim = 5  # number of dimensions
nwalkers = 15  # number of walkers (ndim*2 or 3)
nsteps = 20000 
nburn = 5000

# Initialize walkers in a Gaussian ball around the initial point
initial = np.array([1.0, 7.0, 6.0, -3.8, 1.0])  # initial guess for parameters
...
if __name__ == "__main__":
    ...
    np.savez("chains/new_20000.npz", chain=sampler.get_chain(), log_prob=sampler.get_log_prob())
    flat_samples = sampler.get_chain(flat=True)  # flatten the chain

    print("MCMC sampling completed and saved to new_20000.npz")   # Print completion message

    # Plot the results
    figure = corner.corner(flat_samples, labels=["ge", "gg", "logMe", "gc", "n"], 
                       quantiles=[0.16, 0.5, 0.84], 
                       show_titles=True, smooth=True, title_kwargs={"fontsize": 15})
    plt.savefig("chains/new_20000.pdf")  # Save the corner plot
    plt.show()  