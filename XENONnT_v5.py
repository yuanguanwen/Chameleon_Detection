...
#chameleon model params n, gc, ge (electron/matter coupling), assume rho dominated by electron density
# @njit
...
############################# Bulk Magnetic Field #############################
@njit
def Bsolar(radius_arr, Bup=True):
...
w_array = np.linspace(0.1, 20, 100)  # [keV]
cnv = Rate_cham(w_array, ge=2.0, gg=10.0, logMe=3.67, gc=-3.0, n=1, use_skew=False)  # Gaussian resolution
cnv_skew = Rate_cham(w_array, ge=2.0, gg=10.0, logMe=3.67, gc=-3.0, n=1, use_skew=True)  # skew-Gaussian resolution

Nexp1 = eX1T*np.trapz(cnv, w_array)
Nexp2 = eX1T*np.trapz(cnv_skew, w_array)

end_time1 = time.time()
print(f"Total time taken1: {end_time1 - start_time:.4f} seconds")
print("Gaussian/skew_Gaussian = ", Nexp1, Nexp2)

events = Rate_cham(w_array, ge=2.0, gg=10.0, logMe=3.67, gc=-3.0, n=1, use_skew=False) 
events_skew = Rate_cham(w_array, ge=2.0, gg=10.0, logMe=3.67, gc=-3.0, n=1, use_skew=True) 

end_time2 = time.time()
print(f"Total time taken2: {end_time2 - start_time:.4f} seconds")
...
plt.tight_layout()
# plt.savefig("plots/XENONnT_v5.pdf", dpi=300, bbox_inches='tight')
plt.show()
